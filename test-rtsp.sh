#!/usr/bin/env bash
# HDMI USB RTSP Server Test Script
#
# Comprehensive test suite for rtsp-server.py that validates:
# - HDMI capture device detection (video + audio)
# - RTSP server startup and port binding
# - Client connection and streaming (ffplay/vlc)
# - Error handling and graceful shutdown
#
# Usage: ./test-rtsp.sh
# Prerequisites: HDMI capture device connected, v4l-utils installed, ffplay/vlc available
# Output: Detailed test results with logs saved to test-logs/ directory
...
start_server() {
    ...
    python3 "$RTSP_SCRIPT" --debug > "$SERVER_LOG" 2>&1 &
    SERVER_PID=$!
...
test_client_connection() {
    ...
    if [[ "$client_tool" == "ffplay" ]]; then
        ...
        client_cmd="vlc --intf dummy --play-and-exit --rtsp-tcp '$RTSP_URL'"
    fi
...
check_server_health() {
    ...
    if netstat -ln 2>/dev/null | grep -q ":1234 " || ss -ln 2>/dev/null | grep -q ":1234 "; then
        ...
        if grep -q "Shutting down RTSP server gracefully" "$SERVER_LOG" 2>/dev/null; then
